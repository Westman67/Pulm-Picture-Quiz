import {
  ...
  toggleMarked,
  upsertQualityFlag,
} from "./quiz-core.mjs?v=rename-add-20260917d";
...
async function init() {
  try {
    const [bank, reviewQueue] = await Promise.all([
      fetch("data/question-bank.json?v=rename-add-20260917d").then((response) => response.json()),
      fetch("data/review-queue.json?v=rename-add-20260917d").then((response) => response.json()),
    ]);
