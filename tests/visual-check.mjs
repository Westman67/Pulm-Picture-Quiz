...
const sourceTotals = bank.questions.reduce((totals, question) => {
  totals[question.source_collection_key] = (totals[question.source_collection_key] || 0) + 1;
  return totals;
}, {});
...
if (flagExport.schema_version !== 2 || Object.keys(flagExport.flags || {}).length !== 1) findings.push("Photo-flag export did not contain the saved versioned record");
...
await sourceFilter.locator('select[name="sourceCollection"]').selectOption("third_party");
...
if (!/3rd Party/.test(await sourceFilter.locator(".category-label").textContent())) findings.push("3rd Party collection is not shown on the quiz question");
await sourceFilter.locator('[data-action="home"]').first().click();
await sourceFilter.locator('select[name="sourceCollection"]').selectOption("rename");
await sourceFilter.locator('input[name="length"][value="all"]').check({ force: true });
await sourceFilter.screenshot({ path: shot("desktop-rename-filter.png"), fullPage: true });
await sourceFilter.getByRole("button", { name: /Start ID quiz/ }).click();
await sourceFilter.waitForSelector("#quiz-image");
if (!(await sourceFilter.getByText(`1 / ${sourceTotals.rename}`, { exact: true }).count())) findings.push(`Lecture material collection filter did not produce the expected ${sourceTotals.rename}-question session`);
if (!/Lecture material/.test(await sourceFilter.locator(".category-label").textContent())) findings.push("Lecture material collection is not shown on the quiz question");
